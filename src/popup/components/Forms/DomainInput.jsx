  ...
  return (
    <div className="w-full">
      <div className={`relative flex items-center rounded-[18px] border-[1.5px] py-2 px-2 ${classBorder}`}>
        ...
        <input
          className="appearance-none text-[18px] tracking-[0.02rem] bg-transparent z-50 border-r-2 border-white/10 w-full h-[48px] text-white/60 mr-3 py-1 px-2 leading-tight"
          onFocus={focusInput}
          onBlur={unFocusInput}
          onChange={(e) => { setDomainValue(e.target.value); props.changeValue(e.target.value ? (e.target.value + domainFooter) : "") }}
          value={domainValue}
        />
        <select className="block px-0 text-white bg-[#141416] border-0 peer text-[18px]"
          onChange={(e) => { setDomainFooter(e.target.value); props.changeValue(domainValue + e.target.value) }}
        >
