    ...
    <div className="flex flex-col relative overflow-hidden cursor-pointer
      ...
      <div className="w-full relative">
        ...
        <div className="absolute bottom-[10px] left-[10px] flex flex-row gap-[7px] z-10">
          <div className="h-[20px] w-[20px] rounded-full overflow-hidden"><img src={data.creator.avatar} width={20} height={20} /></div>
          ...
          <div className="flex flex-row items-center">
            {
              data.friends.filter((user, index) => index < 3).map((user, index) => (
                <div className={`flex flex-row ${index !== 0 ? 'ml-[-10px]' : ''}`} key={index}><img src={user.avatar} height={20} width={20} /></div>
              ))
            }
